...
This package is internal to the `you-park` monorepo and is not
...
```
you-park login    [--api-url=URL] [--token=BEARER]
you-park logout
you-park whoami
```
...
```
you-park org list
you-park org use <org-id>
you-park org members
you-park project list
```

Writes (Task 0101):

```
you-park org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
you-park project create <name> [--idempotency-key=KEY]
you-park env create <project-id> <name> [--idempotency-key=KEY]
you-park api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
you-park webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
you-park usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
you-park billing summary
you-park audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`you-park org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `you-park login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/you-park/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/you-park/config.json` (mode 0644, not a secret). Override
both via `YOU_PARK_CONFIG_DIR` (used by tests).
