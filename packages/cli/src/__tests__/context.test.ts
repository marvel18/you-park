...
describe("resolveConfigDir", () => {
  it("respects YOU_PARK_CONFIG_DIR override", () => {
    const prev = process.env["YOU_PARK_CONFIG_DIR"];
    process.env["YOU_PARK_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["YOU_PARK_CONFIG_DIR"];
      else process.env["YOU_PARK_CONFIG_DIR"] = prev;
    }
  });
});
