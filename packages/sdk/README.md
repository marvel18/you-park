...
This package is internal to the `you-park` monorepo. Add it as a
...
All non-2xx responses throw a typed subclass of `YouParkError`. Branch on
...
| `YouParkError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `YouParkError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new YouPark({ fetch })`.
