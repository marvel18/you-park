...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new YouPark({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => YouParkError]> = [
  ...
  it("forward-compat: unknown error code → base YouParkError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(YouParkError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as YouParkError).code).toBe("future_quota_exceeded");
    }
  });
