// Base HTTP transport for the YouPark SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `YouParkError`
 ...
 * `YouPark` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "YouPark SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "YouPark SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
